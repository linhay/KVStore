...
struct KVUserDefaultStore: KVStore {

    static let shared = KVUserDefaultStore(.standard)
    ...
    func get<T>(_ key: String) -> T? where T: KVValue {
        ...
        case .int:
            return store.integer(forKey: key) as? T
        case .uInt:
            let temp = store.integer(forKey: key)
